package com.util.library.view.spinner;
 ...
 */
public class NiceSpinner extends AppCompatEditText {
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        int defaultPadding = typedArray.getDimensionPixelSize(R.styleable.NiceSpinner_defaultPadding, padding);
        int padLeft = typedArray.getDimensionPixelSize(R.styleable.NiceSpinner_textPaddingLeft, 36);

        setGravity(Gravity.CENTER);
        setPadding(padLeft, defaultPadding, defaultPadding, defaultPadding);
        setClickable(true);
        ...
        int maxHeight = typedArray.getDimensionPixelSize(R.styleable.NiceSpinner_dropViewMaxHeight, 0);
        ...
        boolean myEditable = typedArray.getBoolean(R.styleable.NiceSpinner_myEditable, false);
        setEditEnable(myEditable);
    ...
    public boolean onTouchEvent(MotionEvent event) {
        if (isEnabled() && event.getAction() == MotionEvent.ACTION_UP) {
            ...
            if (!editEnable) {
                toggleShowDrawWindow();
            } else {
                if (arrowDrawable != null) {
                    boolean touchable = event.getX() > (getWidth() - getTotalPaddingRight())
                            && (event.getX() < ((getWidth() - getPaddingRight())));
                    if (touchable && adapter.getCount() > 0) {
    ...
    private void toggleShowDrawWindow() {

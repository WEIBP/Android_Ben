package com.util.library.view.mpchart.listener;
...
import com.util.library.view.mpchart.charts.HorizontalBarChart;
...
public class BarLineChartTouchListener extends ChartTouchListener<BarLineChartBase<? extends BarLineScatterCandleBubbleData<?
    ...
    public BarLineChartTouchListener(BarLineChartBase<? extends BarLineScatterCandleBubbleData<? extends
        ...
        isLineChart = chart instanceof LineChart;
    ...
    public boolean onTouch(View v, MotionEvent event) {
        ...
        if ( mChart.isOpenDrawingArea())
            return onTouchWhenDrawArea(v, event);
    ...
    public boolean onTouchWhenDrawArea(View v, MotionEvent event) {
        ...
        switch (event.getAction() & MotionEvent.ACTION_MASK) {
            ...
            case MotionEvent.ACTION_DOWN:
                mTouchMode = mChart.getLineAreaRectBean().checkActionDownInDrawArea(event.getX(), event.getY()) ? AREA_DRAG : AREA_UN_DRAWING;
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (mTouchMode == AREA_DRAG) {
                    float offsetX = event.getX() - mTouchStartPoint.x;
                    float offsetY = event.getY() - mTouchStartPoint.y;
                    mChart.translationArea(offsetX, offsetY);
                } else {
                    mChart.getLineAreaRectBean().setAreaRect(mTouchStartPoint.x, mTouchStartPoint.y, event.getX(), event.getY(), true);
                }
            ...
            case MotionEvent.ACTION_CANCEL:
                if (mTouchMode == AREA_DRAG) mChart.getLineAreaRectBean().resetArea();

                mChart.drawAreaCompleted();
                break;
        }

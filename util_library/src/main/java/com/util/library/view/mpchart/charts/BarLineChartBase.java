...
import com.util.library.common.apachemath.MathUtils;
import com.util.library.view.mpchart.components.XAxis;
...
import com.util.library.view.mpchart.data.LineAreaRectBean;
import com.util.library.view.mpchart.highlight.ChartHighlighter;
import com.util.library.view.mpchart.highlight.Highlight;
import com.util.library.view.mpchart.interfaces.dataprovider.BarLineScatterCandleBubbleDataProvider;
import com.util.library.view.mpchart.interfaces.datasets.IBarLineScatterCandleBubbleDataSet;
import com.util.library.view.mpchart.jobs.AnimatedMoveViewJob;
import com.util.library.view.mpchart.jobs.AnimatedZoomJob;
import com.util.library.view.mpchart.jobs.MoveViewJob;
import com.util.library.view.mpchart.jobs.ZoomJob;
import com.util.library.view.mpchart.listener.BarLineChartTouchListener;
import com.util.library.view.mpchart.listener.OnDrawListener;
import com.util.library.view.mpchart.renderer.LineChartRenderer;
import com.util.library.view.mpchart.renderer.XAxisRenderer;
import com.util.library.view.mpchart.renderer.YAxisRenderer;
import com.util.library.view.mpchart.utils.MPPointD;
import com.util.library.view.mpchart.utils.MPPointF;
import com.util.library.view.mpchart.utils.Transformer;
import com.util.library.view.mpchart.utils.Utils;
...
public abstract class BarLineChartBase<T extends BarLineScatterCandleBubbleData<? extends
    ...
    protected Bitmap chartBgBitmap;
    ...
    private boolean isDrawHighLineValues = true;

    public boolean isDrawHighLineValues() {
        return isDrawHighLineValues;
    }

    public void setDrawHighLineValues(boolean drawHighLineValues) {
        isDrawHighLineValues = drawHighLineValues;
    }
    ...
    private boolean isNeedAutoScale = false;
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mAutoScaleMinMaxEnabled && isNeedAutoScale) {
            autoScale();
        }
        ...
        if (chartBgBitmap != null) {
            canvas.drawBitmap(chartBgBitmap, null, mViewPortHandler.getContentRect(), mGridBackgroundPaint);
        }
...
//            mAxisRendererRight.renderGridLines(canvas);
        ...
        if (valuesToHighlight())
            ((LineChartRenderer) mRenderer).drawHighlighted(canvas, mIndicesToHighlight, isDrawHighLineValues);
        ...
        if (isOpenDrawingArea() && lineAreaRectBean.isShow()) {
            canvas.drawRect(lineAreaRectBean.getStartXWithOffset(), lineAreaRectBean.getStartYWithOffset(),
                    lineAreaRectBean.getEndXWithOffset(), lineAreaRectBean.getEndYWithOffset(), mAreaPaint);
        }
        ...
        if (mLogEnabled) {
            ...
            LogUtils.d("Drawtime: " + drawtime + " ms, average: " + average + " ms, cycles: "
                    + drawCycles);
    ...
    private boolean openDrawingArea = false;

    public boolean isOpenDrawingArea() {
        return openDrawingArea;
    }

    public void setOpenDrawingArea(boolean open) {
        if (openDrawingArea != open) {
            openDrawingArea = open;

            setDragEnabled(!openDrawingArea);
            setScaleEnabled(!openDrawingArea);
            setPinchZoom(!openDrawingArea);
            setDoubleTapToZoomEnabled(!openDrawingArea);

            if (!openDrawingArea) {
                mChartTouchListener.setmTouchMode(0);
            }
    ...
    private LineAreaRectBean lineAreaRectBean = new LineAreaRectBean(mViewPortHandler.getContentRect());

    public LineAreaRectBean getLineAreaRectBean() {
        return lineAreaRectBean;
    }
    ...
    public void setXRange(float xMin, float xMax) {
        if (xMin < xMax && getXAxis().isEnabled()) {
            // TODO: 2020/8/18   设定显示范围  设置默认范围
            getXAxis().mAxisMinimum = xMin;
            getXAxis().mAxisMaximum = xMax;
        }
    }

    public void setYRange(float yMin, float yMax, AxisDependency axis) {
        if (yMin < yMax && getAxis(axis).isEnabled()) {
            getAxis(axis).mAxisMinimum = yMin;
            getAxis(axis).mAxisMaximum = yMax;
        }
    }


    @NotNull
    public ClosedRange<Float> getYRange() {
        float min = 0f,max = 0f;
        if (mAxisLeft.isEnabled()) {
            min = mAxisRendererLeft.yMin;
            max = mAxisRendererLeft.yMax;
            if (mAxisRight.isEnabled()) {
                min = Math.min(min, mAxisRendererRight.yMin);
                max = Math.max(max, mAxisRendererRight.yMax);
            }
        } else if (mAxisRight.isEnabled()) {
            min = mAxisRendererRight.yMin;
            max = mAxisRendererRight.yMax;
        }
        return RangesKt.rangeTo(min,max);
    }
    ...
    public void translationArea(float x, float y) {
        lineAreaRectBean.translationArea(x, y);
    }

    public void drawAreaCompleted() {

        if (drawAreaCompleted != null && isOpenDrawingArea()) {

            MPPointD pointStart = getTransformer(YAxis.AxisDependency.LEFT).getValuesByTouchPoint(getLineAreaRectBean().getStartX(), getLineAreaRectBean().getStartY());
            MPPointD pointEnd = getTransformer(YAxis.AxisDependency.LEFT).getValuesByTouchPoint(getLineAreaRectBean().getEndX(), getLineAreaRectBean().getEndY());
            drawAreaCompleted.invoke(pointStart.x, pointStart.y, pointEnd.x, pointEnd.y);
            MPPointD.recycleInstance(pointStart);
            MPPointD.recycleInstance(pointEnd);
        }
    }

    public MPPointD[] getDrawAreaRange() {
        if (isOpenDrawingArea()) {
            MPPointD pointStart = getTransformer(YAxis.AxisDependency.LEFT).getValuesByTouchPoint(getLineAreaRectBean().getStartX(), getLineAreaRectBean().getStartY());
            MPPointD pointEnd = getTransformer(YAxis.AxisDependency.LEFT).getValuesByTouchPoint(getLineAreaRectBean().getEndX(), getLineAreaRectBean().getEndY());
            return new MPPointD[]{pointStart, pointEnd};
        }
        return new MPPointD[0];
    }

    private Function4<Double, Double, Double, Double, Unit> drawAreaCompleted;

    public void setOnDrawAreaCompleted(@NotNull Function4<Double, Double, Double, Double, Unit> function) {
        drawAreaCompleted = function;
    }

    public void setChartBackgroundResource(Bitmap bgChart) {
        if (chartBgBitmap != null) {
            chartBgBitmap.recycle();
        }
        chartBgBitmap = bgChart;
    }
}

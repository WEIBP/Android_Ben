...
package com.util.library.view.tableview.adapter;
...
public abstract class AbstractTableAdapter<CH, RH, C> implements ITableAdapter<CH, RH, C> {
    ...
    private boolean tableViewDataChanged = false;
    ...
    public List<CH> getmColumnHeaderItems() {
        return mColumnHeaderItems;
    }

    public void setmColumnHeaderItems(List<CH> mColumnHeaderItems) {
        this.mColumnHeaderItems = mColumnHeaderItems;
    }

    public List<RH> getmRowHeaderItems() {
        return mRowHeaderItems;
    }

    public void setmRowHeaderItems(List<RH> mRowHeaderItems) {
        this.mRowHeaderItems = mRowHeaderItems;
    }

    public List<List<C>> getmCellItems() {
        return mCellRecyclerViewAdapter.getItems();
    }
    ...
    public boolean isTableViewDataChanged() {
        return tableViewDataChanged;
    }

    public void setTableViewDataChanged(boolean tableViewDataChanged) {
        this.tableViewDataChanged = tableViewDataChanged;
    }
    ...
    public void initialize() {
       ...
       mColumnHeaderRecyclerViewAdapter.registerAdapterDataObserver(adapterDataObserver);
        mRowHeaderRecyclerViewAdapter.registerAdapterDataObserver(adapterDataObserver);
        mCellRecyclerViewAdapter.registerAdapterDataObserver(adapterDataObserver);
    ...
    public void setCell(int columnPosition,int rowPosition,C cellItem) {

        List<C> cellItems = (List<C>) mCellRecyclerViewAdapter.getItem(rowPosition);
        if (cellItems != null ) {
            if (cellItems.size()>columnPosition){
                // Update cell row items.
                cellItems.set(columnPosition, cellItem);
            } else {
                int count = columnPosition - cellItems.size();
                for (int i = 0; i < count; i++) {
                    cellItems.add(null);
                }
                cellItems.add(cellItem);
            }
            RecyclerView recyclerView = (RecyclerView) mTableView.getCellRecyclerView().getChildAt(rowPosition);
            if (recyclerView != null) {
                recyclerView.getAdapter().notifyItemChanged(columnPosition);
            }
        } else {
            ...
            cellItems.add( cellItem);
            addRow(rowPosition,null,cellItems);
        }
    ...
    public AbstractRecyclerViewAdapter getCellStatisticsRecyclerViewAdapter() {
        return null;
    }
}

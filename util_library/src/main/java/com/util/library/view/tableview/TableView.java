...
package com.util.library.view.tableview;
...
import com.util.library.view.tableview.adapter.AbstractTableAdapter;
import com.util.library.view.tableview.adapter.recyclerview.AbstractRecyclerViewAdapter;
...
import com.util.library.view.tableview.filter.Filter;
import com.util.library.view.tableview.handler.ColumnSortHandler;
import com.util.library.view.tableview.handler.ColumnWidthHandler;
import com.util.library.view.tableview.handler.FilterHandler;
import com.util.library.view.tableview.handler.PreferencesHandler;
import com.util.library.view.tableview.handler.ScrollHandler;
import com.util.library.view.tableview.handler.SelectionHandler;
import com.util.library.view.tableview.handler.VisibilityHandler;
import com.util.library.view.tableview.layoutmanager.CellLayoutManager;
import com.util.library.view.tableview.layoutmanager.ColumnHeaderLayoutManager;
import com.util.library.view.tableview.listener.ITableViewListener;
import com.util.library.view.tableview.listener.TableViewLayoutChangeListener;
import com.util.library.view.tableview.listener.itemclick.ColumnHeaderRecyclerViewItemClickListener;
import com.util.library.view.tableview.listener.itemclick.RowHeaderRecyclerViewItemClickListener;
import com.util.library.view.tableview.listener.scroll.HorizontalRecyclerViewListener;
import com.util.library.view.tableview.listener.scroll.VerticalRecyclerViewListener;
import com.util.library.view.tableview.preference.SavedState;
import com.util.library.view.tableview.sort.SortState;
import com.util.library.common.DpUtil;
...
public class TableView extends FrameLayout implements ITableView {
    ...
    private CellLayoutManager mStatisticsLayoutManager;
    private CellRecyclerView mStatisticsRecyclerView;
    ...
    private boolean mIgnoreSelectionColors = true;
    ...
    private boolean isShowStatisticsView = false;

    private int mStatisticsHeight = DpUtil.dp2px(getContext(), 32 * 4 + 8);

    public boolean isShowStatisticsView() {
        return isShowStatisticsView;
    }

    public void setShowStatisticsView(boolean showStatisticsView) {

        if (isShowStatisticsView == showStatisticsView) return;

        isShowStatisticsView = showStatisticsView;
        mStatisticsRecyclerView.setVisibility(isShowStatisticsView ? VISIBLE : GONE);

        LayoutParams layoutParams = (LayoutParams) mCellRecyclerView.getLayoutParams();
        if (isShowStatisticsView) {
            layoutParams.bottomMargin = mStatisticsHeight + 8;
        } else {
    ...
    public CellRecyclerView getmStatisticsRecyclerView() {
        return mStatisticsRecyclerView;
    }

    public void setmStatisticsRecyclerView(CellRecyclerView mStatisticsRecyclerView) {
        this.mStatisticsRecyclerView = mStatisticsRecyclerView;
    }
    ...
    protected void initializeListeners() {
        ...
        mStatisticsRecyclerView.addOnItemTouchListener(mHorizontalRecyclerViewListener);
        ...
        mStatisticsRecyclerView.addOnLayoutChangeListener(layoutChangeListener);
    ...
    private CellRecyclerView createStatisticsRecyclerView() {
        CellRecyclerView recyclerView = new CellRecyclerView(getContext());
        // Disable multitouch
        recyclerView.setMotionEventSplittingEnabled(false);

        // Set layout manager
        recyclerView.setLayoutManager(getStaticticLayoutManager());

        // Set layout params
        LayoutParams layoutParams = new LayoutParams(LayoutParams.WRAP_CONTENT, mStatisticsHeight);
        ...
        recyclerView.setVisibility(isShowStatisticsView ? VISIBLE : GONE);
    ...
    public <CH, RH, C> void setAdapter(@Nullable AbstractTableAdapter<CH, RH, C> tableAdapter) {
        if (tableAdapter != null) {
            ...
            setStatisticsAdapter(mTableAdapter.getCellStatisticsRecyclerViewAdapter());
    ...
    public void setStatisticsAdapter(AbstractRecyclerViewAdapter adapter) {
        if (adapter != null) {
            mStatisticsRecyclerView.setAdapter(adapter);
        }
    }
    ...
    public CellLayoutManager getStaticticLayoutManager() {
        if (mStatisticsLayoutManager == null) {
            mStatisticsLayoutManager = new CellLayoutManager(getContext(), this);
        }
        return mStatisticsLayoutManager;
    }
    ...
    public void scrollLastToRowPosition(int rowPosition) {

        LinearLayoutManager linearLayoutManager = (LinearLayoutManager) mCellRecyclerView.getLayoutManager();
        ...
        if (visibleItemCount >= rowPosition) {
            mScrollHandler.scrollToRowPosition(0);
        } else {
            mScrollHandler.scrollToRowPosition(rowPosition - visibleItemCount);
        }

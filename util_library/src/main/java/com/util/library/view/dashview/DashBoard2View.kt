 ...
 * description:
 */
class DashBoard2View @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0) : View(context, attrs, defStyleAttr) {
    private var colorDialLower = 0
    private var colorDialMiddle = 0
    private var colorDialHigh = 0
    private var outSaidArcColor = Color_Outside
    ...
    private var mOutSideRect: RectF? = null
    ...
    private var arcRoundRange = 180
    private var maxValue = 100f
    private var minValue = 0f
    private var showValueText = false
    private var showTitleText = false
    private var showScale = false
    private var longScaleLength = 8
    private var shortScaleLength = 1
    private var offsetX = 0
    private var offsetY = 0

    private var strokeWidthDial = 0
    private var pointLineWidth = 13
    private var spaceWidth = 25
    private var pointerWidth = 5
    ...
    fun setArcRoundRange(arcRoundRange: Int) {
        if (arcRoundRange > 300) {
            this@DashBoard2View.arcRoundRange = 300
        }
        if (arcRoundRange < 160) {
            this@DashBoard2View.arcRoundRange = 160
        }
        fitX()
    ...
    private fun initAttrs(context: Context, attrs: AttributeSet?) {
        ...
        showScale = attributes.getBoolean(R.styleable.DashBoardView_show_scale_text, showScale)
        showTitleText = attributes.getBoolean(R.styleable.DashBoardView_show_title_text, showTitleText)
        showValueText = attributes.getBoolean(R.styleable.DashBoardView_show_value_text, showValueText)
    }
    ...
    private fun initPaint() {
        ...
        titlePaint = Paint(Paint.ANTI_ALIAS_FLAG or Paint.FILTER_BITMAP_FLAG)
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        val offset = 25f
        mRect = RectF((-mRealRadius).toFloat() + offset, (-mRealRadius).toFloat() + offset, mRealRadius.toFloat() - offset, mRealRadius.toFloat())

        mOutSideRect = RectF((-mRealRadius).toFloat(), (-mRealRadius).toFloat(), mRealRadius.toFloat(), mRealRadius.toFloat())
        ...
        strokeWidthDial = mRealRadius / 20
        spaceWidth = mRealRadius / 20
        pointLineWidth = mRealRadius / 15

        pointerWidth = (mRealRadius*0.02).toInt()
    ...
    private fun drawArc(canvas: Canvas) {
        ...
        arcPaint!!.color = outSaidArcColor
//        canvas.drawArc(mOutSideRect!!, 270 - 0.5f * arcRoundRange, 0.2f * arcRoundRange, false, arcPaint!!)
//        arcPaint!!.color = colorDialMiddle
//        canvas.drawArc(mOutSideRect!!, 270 - 0.3f * arcRoundRange, 0.6f * arcRoundRange, false, arcPaint!!)
//        arcPaint!!.color = colorDialHigh
//        canvas.drawArc(mOutSideRect!!, 270 + 0.3f * arcRoundRange, 0.2f * arcRoundRange, false, arcPaint!!)

        canvas.drawArc(mRect!!, 180f, 180f, false, arcPaint!!)
    }
    ...
    private fun drawPointerLine(canvas: Canvas) {
        ...
        pointerPaint!!.strokeWidth = pointLineWidth.toFloat()

        val pointerPaintRotate = pointLineWidth * arcRoundRange / (Math.PI.toFloat() * radiusDial)
        //设置起始点的角度
        canvas.rotate(270 - 0.5f * arcRoundRange + pointerPaintRotate / 2)
        for (i in 0..20) {     //一共需要绘制101个表针
            if (i < 7) {
            ...
            canvas.drawLine(mRealRadius.toFloat() - strokeWidthDial  - spaceWidth, 0f,
                    mRealRadius - strokeWidthDial - dp2px(shortScaleLength).toFloat() - spaceWidth - strokeWidthDial , 0f, pointerPaint!!)
            canvas.rotate((arcRoundRange - pointerPaintRotate) / 20f)
        }

//        canvas.rotate(pointerPaintRotate)
...
//        canvas.rotate(90 - 0.51f * arcRoundRange) //恢复坐标系为起始中心位置
    ...
    private fun drawPointer(canvas: Canvas) {
        ...
        pointerPath!!.moveTo(mRealRadius - strokeWidthDial * 2f - spaceWidth*2, 0f)
        pointerPath!!.lineTo(0f, -dp2px(pointerWidth).toFloat())
        pointerPath!!.lineTo(-dp2px(pointerWidth*2).toFloat(), 0f)
        pointerPath!!.lineTo(0f, dp2px(pointerWidth).toFloat())
        pointerPath!!.close()
    ...
    fun setMaxAndMin(max: Float, min: Float) {
        if (max != min) {
            this.maxValue = max(max, min)
            this.minValue = min(max, min)
        }
    }

    companion object {
        private val DEFAULT_COLOR_LOWER = Color.parseColor("#1d953f")
        private val DEFAULT_COLOR_MIDDLE = Color.parseColor("#ECBB65")
        private val Color_Outside = Color.parseColor("#7682C4")
        private const val DEFAULT_COLOR_HIGH = Color.RED
        private val DEAFAULT_COLOR_TITLE = Color.parseColor("#3F4D9B")

...
import com.util.library.common.stringparser.struc.Stack;
 ...
 * </ul>
 * 
 * @see com.util.library.common.stringparser.Expression
 */
public class ExpressionTree {
	...
	public static Expression parse(String s) {
		if (s == null)
			throw new ExpressionParseException(
					"Expression string cannot be null.", -1);
	...
	private static Expression build(String s, int indexErrorOffset) {
		...
		for (int i = 0; i < s.length(); i++) {
			...
			if (term) {
				if (c == '(') {
					if (negate)
						throw new ExpressionParseException(
								"Open bracket found after negate.", i);
					...
					try {
						...
						throw new ExpressionParseException(
								"Improperly formatted value: " + _d, i
										+ indexErrorOffset);
					}
					...
					if (j < s.length()) {
						...
						if (c == '(') {
							...
							while (b != 0) {
								...
								if (k >= s.length()) {
									throw new ExpressionParseException(
											"Missing function close bracket.",
											i + indexErrorOffset);
								}
								...
								if (c == ')') {
									...
									Expression o = build(s.substring(kOld, k),
											kOld);
									if (o == null) {
										throw new ExpressionParseException(
												"Incomplete function.", kOld
														+ indexErrorOffset);
									}
									fn.add(o);
									kOld = k + 1;
					...
					throw new ExpressionParseException("Unexpected character: "
		...
		if (s1.size() != s2.size() + 1) {
			throw new ExpressionParseException("Incomplete expression.",
					indexErrorOffset + s.length());
		}

...
import com.util.library.common.stringparser.func.*;

/**
 * <p>
 * FuncMap maps a name to a function. A FuncMap is used in the eval method of an
 * Expression object. This class can be used as the default function-map. The
 * loadDefaultFunctions() method can be used to take advantage of the many
 * already implemented functions (see below).
 * 
 * <p>
 * During the evaluation of an expression, if a function is not supported then a
 ...
 * <li>tanh(x) &rarr;
 * (<i>e<sup>x</sup>&nbsp;-&nbsp;e<sup>-x</sup></i>)/(<i>e<sup
 * >x</sup>&nbsp;+&nbsp;e<sup>-x</sup></i>)</li>
 ...
 * 
 * <p>
 * Note: Case sensitivity can only be specified in the constructor (for
 * consistency). When case sensitivity is false, the String.equalsIgnoreCase
 * method is used. When case sensitivity is true, the String.equals method is
 * used. The matching does not include the parenthesis. For example, when case
 * sensitivity is false and the default functions have been loaded, then "RaNd",
 * "rand", and "RAND" all map to the RandFunction(). By default, case
...
public class FuncMap {
	...
	/**
	 * Adds the mappings for many common functions. The names are specified in
	...
	/**
	 * Returns a function based on the name and the specified number of
	 * parameters.
	 * 
	 * @throws RuntimeException
	 *             If no supporting function can be found.
	 */
	public Function getFunction(String funcName, int numParam) {
		for (int i = 0; i < numFunc; i++) {
			if (func[i].acceptNumParam(numParam)
					&& (caseSensitive && name[i].equals(funcName) || !caseSensitive
							&& name[i].equalsIgnoreCase(funcName)))
				return func[i];
		}
	...
	public void setFunction(String funcName, Function f) {
		...
		for (int i = 0; i < numFunc; i++) {
			if (caseSensitive && name[i].equals(funcName) || !caseSensitive
					&& name[i].equalsIgnoreCase(funcName)) {
				func[i] = f;
				return;
			}
	...
	/**
	 * Returns an array of exact length of the function names stored in this
	 * map.
	 */
	...
	/**
	 * Returns an array of exact length of the functions stored in this map. The
	...
	/**
	 * Removes the function-name and the associated function from the map. Does
	 * nothing if the function-name is not found.
	 */

...
import com.util.library.common.stringparser.struc.Bag;

/**
 * A node of an expression tree that represents a function. A FuncNode can have
 * 0 or more children.
 */
public class FuncNode extends TermNode {
	...
	/**
	 * Removes the specified expression as a child. Does nothing if the
	 * expression was not a child.
	 */
	...
	/**
	 * Evaluates each of the children, storing the result in an internal double
	 * array. The FuncMap is used to obtain a Function object based on the name
	 * of this FuncNode. The function is passed the double array and returns a
	 * result. If negate is true, the result is negated. The result is then
	 * returned. The numParam passed to the function is the number of children
	 * of this FuncNode.
	 */
